(() => {
  ...
  const icon = L.divIcon({ className: 'person-marker', html: '<div class="person-core" style="--marker-char:&quot;苏&quot;"></div>', iconSize: [28, 28], iconAnchor: [14, 26] });
  ...
  function render() {
    ...
    if (st.status === 'travel') {
      $('clockPhase').textContent = `途中：${E[st.segmentIndex].mapLabel || E[st.segmentIndex].title} → ${E[st.nextIndex].mapLabel || E[st.nextIndex].title}`;
  ...
  function tick(now) {
    ...
    S.last = now;
    // 1× ≈ 1 second of playback per historical year; historical time itself remains linear.

...
- **1–100× 高度夸张**：默认 30×；滑杆与快捷倍率联动。100% = 1×，3000% = 30×。水平位置与距离不变。
- **3D / 2D / 2.5D**：地球、平面地图、倾斜地图；鼠标或触摸旋转、平移、缩放、倾斜。
- **四套地形配色**：山海、经典地形、纸上山川、灰度研究；山体明暗、色彩不透明度和高程色标。
- **城市定位**：内置 Natural Earth 7,342 个城市 / 聚落的索引，可搜索中文、英文或替代名称；支持直接输入 `经度,纬度`。
- **现代参照图层**：国家陆地边界、省 / 州边界、主要河流、经纬网；城市标签按视角避让。
...
3. 打开「省 / 州边界」和「主要河流」，搜索「成都」或 `Chengdu`，把地点放回盆地与山脉中观察。
...
移动端控制面板默认收起，点击面板右侧 `+` 展开。三维渲染需要启用 WebGL 和浏览器硬件加速。
...
- 文件在浏览器本地解析，不上传。**第一版不保存导入文件，刷新页面后需重新导入。**
...
| 主要河流 | Natural Earth 1:50m rivers / lake centerlines；不包含全部河道 |

`1:10m` / `1:50m` 是 Natural Earth 的 **1:1,000 万 / 1:5,000 万制图比例尺**，不是 10 米 / 50 米空间分辨率。

地形覆盖纬度约 **85.05°S–85.05°N**；极帽没有真实高程。源数据的年代、空间分辨率与垂直基准并不统一，海底与陆地精度也不同。第一版最高读取 z12，地形网格为每瓦片 65×65 点；该级别赤道附近几何采样间隔约 153 m，**采样间隔不等于源数据精度**。不能据此声称全球 153 m 分辨率。
...
**本工具显示现代地形与现代参照数据，不提供历史疆域时间轴、板块重建、古海岸线或海平面模拟。** 第一版没有复制 GPlates 的完整数据目录或逐像素视觉效果。地图不是测绘、航海或工程决策产品。

## 开发与验证

应用不需要构建，可直接由 GitHub Pages 发布。所有本地路径相对项目目录，适配 `/terrain-atlas/` 子路径。

本地开发使用 Node.js 20+（检查与测试）和 Python 3（可选静态服务器）：
...
`npm run dev` 在开发者自己的电脑上提供 `http://localhost:4173`。不需要 `npm install`。
...
自动检查不等同于浏览器三维画面验收。不同显卡、浏览器和网络条件下的地形渲染仍应实际打开网页检查。
...
| `src/app.js` | 相机、图层、城市标签、搜索、分享和文件导入 |

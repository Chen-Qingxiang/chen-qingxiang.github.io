import {DEFAULTS, PALETTES, cleanSettings, clamp, parseCoordinates, searchCities, parseGeoJSON, MAX_LATITUDE} from './core.js';
...
function initialState() {
  ...
  return {settings: cleanSettings(shared?.settings || saved?.settings || DEFAULTS), camera: shared?.camera || null};
}
...
export async function start(C) {
  ...
  function setPanel(open) {
    ...
    document.body.classList.toggle('panel-is-collapsed', !open);
  ...
  if (window.matchMedia('(max-width:760px)').matches) setPanel(false);
  ...
  function replaceOverlays() {
    const layers = [];
    if (settings.rivers && datasets.has('rivers')) layers.push({lines: datasets.get('rivers'), color: '#397f9b', width: 2.2});
  ...
  function updateScale() {
    ...
    $('scale').style.setProperty('--fill', `${(settings.scale-1)/99*100}%`);
  ...
  for (const key of ['relief','cities','countries','provinces','rivers','grid']) {
    $(key).checked = settings[key];
    if (['countries','provinces','rivers'].includes(key)) $(key).disabled = true;
    $(key).onchange = async event => {
      ...
      if (['countries','provinces','rivers'].includes(key) && settings[key] && !datasets.has(key)) {
  ...
  function lookAtCenter(pitch, north=false) {
    ...
    camera.flyToBoundingSphere(new C.BoundingSphere(center,1),{offset:new C.HeadingPitchRange(north?0:camera.heading,pitch,range),duration});
  }
  ...
  function refreshCities() {
    if (!cityData.length || scene.mode===C.SceneMode.MORPHING) return;
    const wanted = new Set();
    if (settings.cities) {
      const rect=camera.computeViewRectangle(scene.globe.ellipsoid);
      const height=camera.positionCartographic.height;
      const rank=height>10000000?2:height>4000000?4:height>1200000?7:10;
      const occluder = new C.EllipsoidalOccluder(scene.globe.ellipsoid,camera.positionWC);
      const boxes=[];
      for (const point of cityData) {
        if (point.rank>rank || Math.abs(point.lat)>MAX_LATITUDE) continue;
        if (rect && !C.Rectangle.contains(rect,C.Cartographic.fromDegrees(point.lon,point.lat))) continue;
        const pos=C.Cartesian3.fromDegrees(point.lon,point.lat);
        if (scene.mode===C.SceneMode.SCENE3D && !occluder.isPointVisible(pos)) continue;
        const screen=C.SceneTransforms.worldToWindowCoordinates(scene,pos);
        if (!screen || screen.x<0 || screen.y<0 || screen.x>scene.canvas.clientWidth-35 || screen.y>scene.canvas.clientHeight-25) continue;
        const width=Math.min(180,(point.zh||point.name).length*10+16);
        if (boxes.some(b => Math.abs(b.y-screen.y)<28 && screen.x<b.x+b.w && screen.x+width>b.x)) continue;
        boxes.push({x:screen.x,y:screen.y,w:width});
        const id=`city-${point.id}`; wanted.add(id);
        if (!cityEntities.has(id)) cityEntities.set(id,entityForPoint(point,id));
        if (wanted.size>=65) break;
      }
    }
    for (const [id,entity] of cityEntities) if (!wanted.has(id)) {viewer.entities.remove(entity);cityEntities.delete(id);}
    requestRender();
  }
  camera.moveEnd.addEventListener(() => {clearTimeout(refreshTimer);refreshTimer=setTimeout(refreshCities,180);});
  ...
  handler.setInputAction(event => {
    if(performance.now()-lastMove<80)return;lastMove=performance.now();
  ...
  if(validCamera(initial.camera)) {
    const p=initial.camera;camera.setView({destination:C.Cartesian3.fromDegrees(p.lon,p.lat,p.height),orientation:{heading:p.heading,pitch:p.pitch,roll:p.roll}});
  ...
  try {
    ...
    await Promise.all([
      ...
      ...['countries','provinces','rivers'].filter(k=>settings[k]).map(async key=>{await ensureDataset(key);replaceOverlays();}),
    ]);
  }catch(error){notify(error.message,7000);$('search-hint').textContent='城市数据未就绪；仍可输入经纬度定位。';}
  if (manifest) ['countries','provinces','rivers'].forEach(key => {$(key).disabled=false;});
